...
"""PyTorch Qwen2Audio model, modified from https://github.com/huggingface/transformers/blob/v4.53.2/src/transformers/models/qwen2_audio/modeling_qwen2_audio.py."""
...
from modeling_qwen2 import Qwen2ForCausalLM as CustomQwen2ForCausalLM
...
class Qwen2AudioAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # IMPORTANT: Forced to SDPA
        attention_interface: Callable = eager_attention_forward
        if self.config._attn_implementation != "eager":
            attention_interface = ALL_ATTENTION_FUNCTIONS["sdpa"]
            # attention_interface = ALL_ATTENTION_FUNCTIONS[self.config._attn_implementation]

        # print('in qwen2-audio attention forward:')
...
class Qwen2AudioForConditionalGeneration(Qwen2AudioPreTrainedModel, GenerationMixin):
    def __init__(
        ...
        enable_partial_yarn: bool = False,
        interpolation_start_dim: int = 0,
        partial_interpolation_attention_temperature: float = 1.0,
    ):
        super().__init__(config)
        ...
        # Partial Yarn: modify base llm.
        language_model_config = config.text_config
        language_model_cls = transformers.Qwen2ForCausalLM
        if enable_partial_yarn:
            language_model_cls = CustomQwen2ForCausalLM
            language_model_config.interpolation_start_dim = interpolation_start_dim
            language_model_config.partial_interpolation_attention_temperature = (
                partial_interpolation_attention_temperature
            )

        self.language_model = language_model_cls(language_model_config)
            ...
            if input_features is not None and input_ids.shape[1] != 1:
                ...
                if legacy_processing:
                    ...
                    special_audio_mask = special_audio_mask.unsqueeze(-1).expand_as(
                        inputs_embeds
                    )  # batch, seq, expanded dim

                    audio_features = audio_features.to(inputs_embeds.device, inputs_embeds.dtype)
                    inputs_embeds = inputs_embeds.masked_scatter(special_audio_mask, audio_features)

                    special_audio_mask = special_audio_mask[..., 0]

        outputs = self.language_model(
            special_audio_mask=special_audio_mask,  # Partial Yarn: Mask of audio positions.
            attention_mask=attention_mask,
