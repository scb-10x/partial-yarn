...
"""
Processor class for Qwen2Audio. Modified from https://github.com/huggingface/transformers/blob/v4.53.2/src/transformers/models/qwen2_audio/processing_qwen2_audio.py.
"""
        ...
        if audio is not None:
            ...
            for sample in text:
                ...
                while "<placeholder>" in sample:
                    ...
                    # Partial Yarn: make audio chunks directly connected
                    sample = sample.replace("<|AUDIO|><|audio_bos|>", "<|AUDIO|>")
                    sample = sample.replace("<|audio_eos|><|AUDIO|>", "<|AUDIO|>")
                    sample = sample.replace("<|audio_eos|><|audio_bos|>", "")
        ...
        * If the content element is an audio, the template will output a sequence of <|AUDIO|> tokens
        ...
        return (
            ...
            "{% for message in messages %}\n"
                ...
                "{% else %}\n"
                    "{% set current_message_audio_count = namespace(value=0) %}\n"
                    "{% set ns_text_parts = namespace(value=[]) %}\n"
                    "{% for content in message['content'] %}\n"
                        "{% if 'audio' in content or 'audio_url' in content or message['type'] == 'audio' or content['type'] == 'audio' %}\n"
                            "{% set current_message_audio_count.value = current_message_audio_count.value + 1 %}\n"
                        "{% elif 'text' in content %}\n"
                            "{% set ns_text_parts.value = ns_text_parts.value + [content['text']] %}\n"
                        "{% endif %}\n"
                    "{% endfor %}\n"
                    "{% if current_message_audio_count.value > 0 %}\n"
                        "{% set audio_count.value = audio_count.value + 1 %}\n"
                        "Audio {{ audio_count.value }}: <|audio_bos|>{{ '<|AUDIO|>' * current_message_audio_count.value }}<|audio_eos|>\n"
                    "{% endif %}\n"
                    "{{ ns_text_parts.value | join('') }}<|im_end|>\n"
                "{% endif %}\n"
            "{% endfor %}\n"
